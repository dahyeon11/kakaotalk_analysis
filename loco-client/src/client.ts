...
export class LocoClient {
  ...
  /** Full login sequence: connect → RSA handshake → CHECKIN → LOGINLIST → SETST → GETTOKEN */
  async login(): Promise<Record<string, unknown>> {
    ...
    console.log("[LocoClient] CHECKIN response:", JSON.stringify(checkinResp.body, null, 2));
